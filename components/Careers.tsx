...
import { Briefcase, MapPin, Clock, Calendar, DollarSign, Terminal, Send, CheckCircle, Mail } from 'lucide-react';
...
const Careers: React.FC = () => {
  ...
  const handleNewsletterSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const subject = encodeURIComponent('Notify me about Lipila career opportunities');
      const body = encodeURIComponent(`Please notify me about future opportunities at Lipila Technologies.\n\nEmail: ${newsletterEmail}`);
      window.location.href = `mailto:service@lipila.tech?subject=${subject}&body=${body}`;
  ...
  return (
    <section id="careers" className="py-24 bg-slate-50 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Job Listings or Closed Message */}
        {jobs.length > 0 ? (
          ...
          <div className="max-w-3xl mx-auto mb-24 bg-white p-10 lg:p-16 rounded-[3rem] shadow-xl border border-slate-100 text-center">
            <div className="w-20 h-20 bg-indigo-50 text-indigo-600 rounded-full flex items-center justify-center mx-auto mb-6">
              <Briefcase className="w-10 h-10" />
            </div>
            <h3 className="text-3xl font-bold text-slate-900 mb-3">We're Not Hiring Right Now</h3>
            <p className="text-lg text-slate-600 mb-8">We don't currently have open positions, but we're always interested in connecting with talented people who share our passion for building technology that matters in Africa.</p>
            <p className="text-slate-600 mb-8">Drop your email below and we'll reach out when new opportunities become available.</p>
          </div>
        )}

        {/* Application Form or Newsletter Signup */}
        {jobs.length > 0 ? (
          ...
          <div id="newsletter-signup" className="max-w-2xl mx-auto bg-white p-10 lg:p-16 rounded-[3rem] shadow-2xl relative overflow-hidden">
            <div className="absolute top-0 left-0 w-2 h-full bg-indigo-600"></div>
            ...
            {newsletterSubmitted ? (
              <div className="text-center py-12 animate-in fade-in duration-500">
                ...
                <p className="text-slate-600">We'll notify you as soon as new opportunities open up. Thanks for your interest in Lipila Technologies!</p>
              </div>
            ) : (
              <>
                <div className="mb-8">
                  <h3 className="text-3xl font-bold text-slate-900 mb-3">Get Notified of New Opportunities</h3>
                  <p className="text-slate-600">Be among the first to hear when we're hiring. Leave your email and we'll keep you posted.</p>
                </div>
                <form onSubmit={handleNewsletterSubmit} className="space-y-4">
                  ...
                  <button
                    ...
                    className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-bold text-lg hover:bg-indigo-700 shadow-xl flex items-center justify-center transition-all disabled:opacity-70"
                  >
